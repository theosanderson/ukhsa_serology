...
library(lubridate)
library(viridisLite)
data<-read_csv("combined.csv") %>% separate(month,into=c("month","year")) %>% mutate(date= lubridate::dmy(paste(15,month,year)))

old_data<-read_csv("old_data.csv") %>% separate(month,into=c("month","year")) %>% mutate(date= lubridate::dmy(paste(15,month,year)))
nrow(old_data)
...
a <- animate(anim, renderer = file_renderer("/Users/sandert/ooutput_movie6/"),height=500*1.5,width=800*1.5,res=180,nframes = 500)
